...
def output_to_target(output, width, height):
    ...
    if isinstance(output[0], torch.Tensor):
        output = list(map(lambda e:e.cpu().numpy(), output))
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None, rect_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    if rect_thickness is None:
        rect_thickness=tl
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, color, thickness=rect_thickness, lineType=cv2.LINE_AA)
    if label:
