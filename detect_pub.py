...
class LoadTopic:  # for inference
    def __init__(self, rgb_image_topic, img_size=640):
        ...
        self.pub_yolo_result = rospy.Publisher('/dsr_tray/yolo_result', msgYolo, queue_size=5)
        # self.pub_yolo_image = rospy.Publisher('/dsr_tray/yolo_image', msgImage, queue_size=3)
        self.yolo=msgYolo()
    ...
    def imageRGBCallback(self,Image): 
        try:
            ...
            # print(cv_rgbimg.shape)
            # self.img=cv2.resize(cv_rgbimg, (0,0), fx=16/9, fy=16/9)
            self.img=cv_rgbimg
...
def detect(weights='', imgsz=640, conf_thres=0.4, iou_thres=0.5, dev='', agnostic_nms=False, augment=False):
    ...
    if weights == '':
        # weight_path=os.path.dirname(os.path.abspath(__file__))+ '/runs/exp22_yolov4-csp-results/weights/' + 'best.pt'#'best_yolov4-csp-results.pt'
        weight_path=os.path.dirname(os.path.abspath(__file__))+ '/runs/exp22_yolov4-csp-results/weights/' + 'best_yolov4-csp-results.pt'
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            for det in pred:  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    dataset.pub_yolo_result.publish(dataset.yolo) #publish original image to find handle
            
            # dataset.pub_yolo_image.publish(dataset.bridge.cv2_to_imgmsg(im0, encoding='bgr8'))
            cv2.imshow('detection', cv2.resize(im0, (0,0), fx=0.5, fy=0.5))
            cv2.waitKey(1)
...
def main():
    global dataset
    rgb_image_topic= "/global_camera/color/image_raw"
...
if __name__=='__main__':
    ...
    th2 = Thread(target=detect, args=('', 480, 0.4, 0.5, '', False, False), daemon=True)
